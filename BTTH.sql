...
SELECT hotels.name AS "Khách sạn", hotels.rate AS "Sao(rate)", SUM(bookings.revenue) AS "Doanh thu",
COUNT(CASE WHEN bookings.status = "COMPLETED" THEN 1 END) AS "Đặt(Đã thanh toán)",
COUNT(CASE WHEN bookings.status = "CANCELLED" THEN 1 END) AS "Hủy"
FROM users
